...
void DecoyAction::execute_impl() {
	...
	if (timeRemaining == -1) {
		...
		// if (characterPerformingAction.getMana() >= spell.getMana()) {// no spell
        if (characterPerformingAction->getMana() >= 0) {
			auto newDecoy = characterPerformingAction; // copy constructor?
			ourDecoyId = newDecoy->getEntityId(); // to bind decoy to this action obj
			gameState.addCharacter(*newDecoy, room->getId());
            logger->debug("decoy added in room " + std::to_string(room->getId()));
		} else {
			logger->debug("Spell not cast, not enough mana");
		}
	} else if (timeRemaining == 0) {		
		gameState.removeCharacterByUniqueId(ourDecoyId);
        logger->debug("decoy removed from room " + std::to_string(room->getId()));
	} else {
