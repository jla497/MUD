 ...
 * the server*/
void ConnectionManager::receiveFromGameManager(const std::vector<gameAndUserInterface> &fromGame) {

    for (auto& msg : fromGame) {
        auto it = std::find_if(mList.begin(), mList.end(), findContainer(msg.conn));
        if (it == mList.end()) {
